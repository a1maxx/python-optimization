...
model.EPS = pyo.Param(initialize=1.05)
model.CG = pyo.Param(initialize=10)
model.CRG = pyo.Param(initialize=10)
model.CEM = pyo.Param(initialize=10)
model.CS = pyo.Param(initialize=10)
model.CD = pyo.Param(initialize=10)
model.PIC = pyo.Param(initialize=10)
model.PID = pyo.Param(initialize=10)
...
# Phase difference between Vit and Vjt
...
model.soc = pyo.Var(model.A, model.T)
model.n1 = pyo.Var()
...
model.x = pyo.Var(model.A, model.A)
# Connection status of the load at time t
model.y = pyo.Var(model.A, within=pyo.Binary)


def obj_expression(m):
    return sum(sum(m.v[k, t] - m.VN + sum(m.x[k, j] for j in m.K) + m.PRI[k] * m.PD[k] * (1 - m.y[k, t])
                   for k in m.K) for t in m.T)
...
def constraint_rule32(m, k, t):
    return 1 - m.EPS <= m.v[k, t] & m.v[k, t] <= 1 + m.EPS
...
model.cons35 = pyo.constraint(model.A, model.A, model.T, rule=constraint_rule35)
...
model.cons36 = pyo.constraint(model.A, model.A, model.T, rule=constraint_rule36)
...
model.cons37 = pyo.Constraint(model.A, model.T, rule=constraint_rule37)


def constraint_rule38(m, k, t):
    return m.pe[k, t] <= 200 * m.phi[k, t] & -m.pe[k, t] * m.lamb[k, t] <= m.pe[k, t]
...
def constraint_rule41(m, k, t):
    return 0 <= m.soc[k, t] & m.soc[k, t] <= 100
...
def constraint_rule43(m, t):
    return sum(m.q[k, t] + m.QR[k, t] for k in m.A) >= sum(m.QD[k, t] for k in m.A)
